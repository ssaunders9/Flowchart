/**
 * WSU Engineering Curriculum Flowchart - Main Application Logic
...
// ✅ PHASE 2.1: Centralized configuration constants
const CONFIG = {
...
// ✅ PHASE 2.2: LaneRouter class for managing lane reservations and conflicts
class LaneRouter {
...
function initDOMCache() {
    ...
    domCache.welcomeBanner = document.getElementById('welcomeBanner');
    domCache.welcomeDismissBtn = document.getElementById('welcomeDismissBtn');
...
/**
 * Focus trap for the side pane - keeps Tab/Shift+Tab cycling within the pane
 */
let _paneFocusTrapHandler = null;

function setupPaneFocusTrap(pane) {
    removePaneFocusTrap(); // Clean up any existing trap
    
    _paneFocusTrapHandler = function(e) {
        if (e.key !== 'Tab') return;
    ...
    pane.addEventListener('keydown', _paneFocusTrapHandler);
}

function removePaneFocusTrap() {
    if (_paneFocusTrapHandler) {
        const pane = domCache.courseInfoPane;
        if (pane) {
            pane.removeEventListener('keydown', _paneFocusTrapHandler);
        }
        _paneFocusTrapHandler = null;
    }
}
...
function highlightLeadsTo(leadsToKeys) {
    leadsToKeys.forEach(leadsToKey => {
        ...
        applyRelationshipStyle(leadsToDiv, 'leadsto', '▶');
    });
}
...
function drawAllConnections() {
    // ✅ PHASE 1.2: Start performance measurement
...
return;
    ...
    // ✅ PHASE 1.2: DocumentFragment for batch rendering - collects all SVG elements before single append
    ...
    // ✅ PHASE 1.2: Helper to create SVG elements with attributes
...
return aRect.centerX - bRect.centerX;
    ...
    // ✅ PHASE 1.3: Performance tracking for conflict detection
    performance.mark('conflict-detection-start');

    // Helper: Check if a vertical lane segment is available
    // ✅ PHASE 1.3: Optimized with early exits and reduced redundant calculations
...
return true;
    ...
    // ✅ PHASE 1.3: Optimized with early exits
...
globalHorizontalLanes[fromCourseKey] = {};
    ...
    // ✅ PHASE 2.3: Create LaneRouter instances for each gap (vertical lanes)
...
if (gap && gap.lanes) {
    ...
    // ✅ PHASE 2.3: Create LaneRouter instances for horizontal lanes
...
if (jump === 0) sameSemesterCount++;
    ...
    // ✅ PHASE 1.2: Append all elements at once (single paint)
    svg.appendChild(svgFragment);

    // ✅ PHASE 1.3: End conflict detection tracking
    ...
    // ✅ PHASE 1.2: End performance measurement
    ...
    semesterOrder.forEach(semester => {
        ...
        courseBySemester[semester].forEach(course => {
            ...
            const alternativesText = course.alternatives && course.alternatives.length > 0
                ? ` Has ${course.alternatives.length} alternative course${course.alternatives.length > 1 ? 's' : ''}.`
                : '';
...
window.switchProgram = function() {
    ...
    // ✅ FIX #1: COMPREHENSIVE CACHE CLEANUP (prevents memory leaks)
    cleanupAllCaches();
...
// Keyboard handler is consolidated in setupKeyboardNavigation() below
...
function populateProgramSelector() {
    ...
    const programs = Object.keys(programsData).map(key => {
        ...
        // Warn about missing data in development
...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        // Initialize welcome banner (hide if previously dismissed)
        try {
            initWelcomeBanner();
        } catch (error) {
            console.error('Error initializing welcome banner:', error);
        }
    ...
    setTimeout(() => {
        ...
        try {
            ...
            const welcomeDismissBtn = domCache.welcomeDismissBtn;
    ...
    semesterOrder.forEach((semester, semesterIndex) => {
        ...
        courseBySemester[semester].forEach(course => {
            ...
            if (course.semesterRestriction) {
                ...
                restrictionP.style.color = '#7B3D14'; // Darkened for AAA contrast
...
// Welcome / Getting Started banner
function initWelcomeBanner() {
    const banner = domCache.welcomeBanner;
    if (!banner) return;

    const dismissed = safeLocalStorageGet('flowchartWelcomeDismissed');
    if (dismissed === true) {
...
window.dismissWelcomeBanner = function() {
    ...
    if (!banner) return;
    banner.classList.add('hidden');
    safeLocalStorageSet('flowchartWelcomeDismissed', true);
    // Move focus to the guide toggle so user has a clear next step
    const guide = domCache.guideToggle;
...
window.toggleView = function() {
    ...
    if (currentView === 'visual') {
        ...
        if (announcer) {
            announcer.textContent = 'Switched to list view. Courses are listed by semester with prerequisites, corequisites, and required-for relationships. Use heading navigation to jump between semesters.';
        }
...
window.highlightCourseLines = function(courseKey) {
    ...
    requestAnimationFrame(() => {
        ...
        if (selectedCourse) {
            ...
            // Priority 3: ▶ badges to courses this course leads to
...
window.clearHighlights = function() {
    ...
    // Clear connection tracking to prevent memory buildup
    connectionCounts = {};
    sourceConnectionCounts = {};
    if (window.connectionBoundingBoxes) {
        window.connectionBoundingBoxes.clear();
...
function getCurrentCourse() {
    return document.activeElement.closest('.course[data-course-key]') || null;
}
...
function focusCourse(courseKey) {
    ...
    if (courseDiv) {
        courseDiv.focus();
        const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
        courseDiv.scrollIntoView({ behavior: prefersReducedMotion ? 'auto' : 'smooth', block: 'nearest', inline: 'nearest' });
...
function setupKeyboardNavigation() {
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'v' || e.key === 'V') {
            // Don't toggle view if focus is inside the course info pane
            const pane = domCache.courseInfoPane;
            if (pane && pane.contains(document.activeElement)) return;
            // Close pane before switching views
            if (pane && pane.getAttribute('aria-hidden') === 'false') {
                window.closeCourseInfo();
                window.clearHighlights();
            }
